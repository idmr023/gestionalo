<x-layouts.admin :title="$project ? 'Editar Proyecto' : 'Nuevo Proyecto'">
    <div class="max-w-2xl mx-auto">
        ...
        <form wire:submit="save" class="bg-white border border-primary/10 p-8 space-y-6">
            ...
            <div>
                <label for="subtitle" class="block text-sm font-medium text-primary/70 mb-2">Subtítulo / Tipo de negocio</label>
                <input id="subtitle" type="text" wire:model="subtitle" placeholder="Ej: Planta Industrial" class="w-full border border-primary/10 px-4 py-2.5">
                @error('subtitle') <p class="text-accent text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="description" class="block text-sm font-medium text-primary/70 mb-2">Descripción (opcional)</label>
                <textarea id="description" wire:model="description" rows="4" class="w-full border border-primary/10 px-4 py-2.5"></textarea>
            ...
            <div>
                ...
                @if ($logo_path)
                    <div class="mb-3 inline-block p-3 bg-surface">
                        <img src="{{ Storage::url($logo_path) }}" alt="Logo actual" class="h-16 w-auto object-contain">
                    </div>
                @endif
